 ...
 * flow. It is dedicated to describe current transaction and represent a transaction
 * on the type level.
 *
 * @warning Depending on the underlying `%Connection` the transaction object may model `Nullable`.
 *          If the object is in null state call of any member may lead to undefined behavior.
 *
 ...
 * @par Thread safety
 *
 * *Distinct objects*: Depends on underlying `%Connection` model.
 *
...
public:
     ...
     *
     * @note Constructing a transaction object itself does not open any transaction,
     ...
     */
    transaction(Connection&& connection, Options options)
     ...
     */
    bool is_open() const noexcept { return lowest_layer().is_open();}
